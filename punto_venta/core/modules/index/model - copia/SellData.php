...
class SellData {
 ...
 public static function getById($id){
  ...
  public static function getAllApplied($user){
    $sql = "select * from ".self::$tablename." where mesero_id=$user and is_applied=1 order by created_at desc";
    $query = Executor::doit($sql);
    return Model::many($query[0],new SellData());
  ...
  public static function getAllByuser($user,$dia){
    $sql = "select * from ".self::$tablename." where mesero_id=$user and created_at=$dia";
    $query = Executor::doit($sql);
    return Model::many($query[0],new SellData());
